...
const SaraleScreen = ({ route }: { route: any }) => {
  const { data } = route.params;

  if (!data || !Array.isArray(data)) {
    return <ListWithIcons title="Sarali Varsai" items={["No data available"]} />;
  }

  const items = data.map((item: any) => item.name); // Or item.notation, based on your JSON

  return <ListWithIcons title="Sarali Varsai" items={items} />;
